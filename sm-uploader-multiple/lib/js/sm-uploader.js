...
jQuery(document).ready(function($){
    ...
    $('#galleryArea a.remove').on('click', function(){
        ...
        var removeTarget = setTimeout(function(){
            // if left only 1, disable delete button.
            target.remove(); 
            chk_img_amount();
...
function chk_img_amount(){
    // if left only 1, disable delete button.
    if($("#sortable").children("li").length <= 1){
        $("#sortable").children("li").find(".addimg").find("a.remove").css("display", "none");
    }else{
        $("#sortable").children("li").find(".addimg").find("a.remove").css("display", "inline");
    }    
}
